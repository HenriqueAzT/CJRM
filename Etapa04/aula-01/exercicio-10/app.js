...
let cat = {
  ...
  age: 2,
  color: 'gray',
  bestFriends: ['Valtinho', 'Aparecido'],
  sound: function() {
    return 'Miau'
  }
}
...
const isAnSUV = car => {
  const hrv = car === 'Honda HR-V'
  const renegade = car === 'Jeep Renegade'
  const ecoSport = car === 'Ford EcoSport'
  const ix35 = car === 'Hyundai iX35'

  if (hrv || renegade || ecoSport || ix35) {
    return true
  }
