...
// console.log(age); // Esse erro acontece devido ao escopo da let age, que foi declarada dentro da função, e portanto só pode ser usada dentro da mesma
...
let car = {
  ...
  run () {
    this.isRunning = true
    console.log(`A ${car.name} está em movimento`);
  },
  stop () {
    this.isRunning = false
    console.log(`A ${car.name} está parado`);
  },
  getColorsMessage() {
    return `A ${car.name} está disponível nas cores ${car.colors[0]}, ${car.colors[1]} e ${car.colors[2]}`
  }
}
