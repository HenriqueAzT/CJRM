...
const logFetchGitHub = async username => {
  console.log(await fetchGitHubUser(username));
}

logFetchGitHub('HenriqueAzT')
...
const getdisisibleBy2Or3 = numbers => numbers
  .filter(number => number % 2 === 0 || number % 3 === 0)

console.log(getdisisibleBy2Or3(numbers));
...
const myName = ['He', 'n', 'ri', 'que']

const nameInPLanguage = myName.reduce((acc, syllable) => `${acc}P${syllable}`, '')
...
const searchInArray = (array, value) => 
  array.reduce((acc, item) => value === item ? acc + 1 : acc, 0)

console.log(searchInArray(scores, 100));
