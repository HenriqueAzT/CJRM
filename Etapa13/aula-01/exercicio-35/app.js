...
const myName = {x: 'Henrique', y: 'Azara'}

localStorage.setItem('myKey', JSON.stringify(myName))

const JSONFromLS = localStorage.getItem('myName')
const myConvertedConst = JSON.parse(JSONFromLS)

console.log(myConvertedConst);
...
const combineOperations = (number, arrayOfFuncs) => 
  arrayOfFuncs.reduce((acc, func) => func(acc), number)
...
const searchAlbumExistsInArray = albums
  .filter(album => album.id === searchAlbum.id)
...
// const objJSON = JSON.stringify(obj) // Também copia, mas a prop0 não aparece no console
// const objCopy = JSON.parse(objJSON)

// const objCopy = { ...obj } // Copia e a prop0 aparece no console, porém é uma cópia rasa, não funciona com objetos e arrays, como o prop8 e o prop6, respectivamente
