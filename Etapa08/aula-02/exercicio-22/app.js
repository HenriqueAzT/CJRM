...
const namesInOrder = getArrayCopy(names).sort()
...
const higherThan50 = randomNumbers.find(number => number > 50)
...
// const movieNames = topBrazilmovies   FORMA DE FAZER COM O MAP 
//   .map(item => `<li>${item.title}</li>`)
//   .join('')

const movieNames = topBrazilmovies
  .reduce((acc, {title}) => acc + `<li>${title}</li>`, '') // FORMA DE FAZER COM O REDUCE
