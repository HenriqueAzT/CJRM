...
for(let k = 0; k < randomValues.length; k++){
    const item = randomValues[k]
    const typeOfItem = typeof item
    const isItemAString = typeOfItem === 'string'
    const isItemABool =  typeOfItem === 'boolean'
...
lastIndexOfRandomValues = firstFourStrings[firstFourStrings.length - 1]
const fourStrings = firstFourStrings.join(', ').replace(`, ${lastIndexOfRandomValues}`, ` e ${lastIndexOfRandomValues}`)

console.log(`3 informações sobre o array randomValues:
    - As primeiras 4 strings são ${fourStrings};
    - Até que as primeiras 4 strings fossem iteradas, ${boolCounter} booleans foram iterados;
    - O array foi iterado por ${numIterations} vezes.`)
