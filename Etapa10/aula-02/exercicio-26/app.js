...
const formatTimeUnit = (value) => String(value).length === 1 ? `0${value}` : value

const formatDate = () => {
  const Day = formatTimeUnit(present.getDay())
  const Month = formatTimeUnit(present.getMonth() + 1)
...
const myBirth = new Date('April 28 2003 22:00:00')

const myBirthFormat = () => {
  const hours = myBirth.getHours()
  const minutes = myBirth.getMinutes()
  const exactTime = `${formatTimeUnit(hours)}:${formatTimeUnit(minutes)}`
  const day = weekDays[myBirth.getDay()]
  const month = monthNames[myBirth.getMonth()]
  const monthDay = myBirth.getDate()
  const year = myBirth.getFullYear()

  return `${exactTime} - ${day}, ${monthDay} de ${month} de ${year}`
}

console.log(myBirthFormat());
...
const updater = ({target, property, willChange}) => {
...
const updateSomething = (data = {}) => updater(data)
